...
# -*- coding: utf-8 -*-
"""
Created on Sun Jul 26 14:52:21 2020
...
#resample_dic={'act':'mean', 'HR':'mean', 'var':'mean', 'post':get_max,'Score':get_max}
#path='/Users/hao/Desktop/python/sleep_data_process/2_stage/health_2'
#name='20200408'
#data=pd.read_excel(path+'/'+name+'.xlsx',sheet_name='whole data')
#data=data.set_index('Time')
##data=data.resample('30s').agg(resample_dic)
#data=data.resample('30s').mean()
#data=data.fillna(method='ffill')
#data=data.reset_index(inplace=False)
#data=data.rename(columns={'index':'Time'})
#data.to_excel(name+"_30s.xlsx",sheet_name='whole data')
import pandas as pd
import os
...
def filter_wake_sleep(x,y,threshold):
    ...
    return output
#%%
path='/Users/hao/Desktop/python/sleep_data_process/2_stage/health_30s'
...
#tr_score = tr.decision_function(act_test.reshape(-1, 1))
#fpr_tr, tpr_tr,_=roc_curve(score_test.ravel(),tr_score.ravel())
...
plot_confusion_matrix(confusion_matrix(score_test,tr_predictions),classes=my_yticks1,
                      title='Suppert vector machine',xlabel=True,ylabel=True)
